...
# original version from https://github.com/mayorx/rcf-edge-detection
class BSDS_RCFLoader_original(data.Dataset):
    ...
    def __init__(self, root='data/bsds', split='train', transform=False):
        ...
        if self.split == 'train':
            ...
            self.filelist = join(self.bsds, 'test_list_only_img.lst')
        else:
            raise ValueError("Invalid split type!")
        with open(self.filelist, 'r') as f:
            self.filelist = f.readlines()
    ...
    def __getitem__(self, index):
        ...
        if self.split == "train":
            ...
            original_img = np.array(cv2.imread(join(self.bsds, img_file)), dtype=np.float32)
            img = prepare_image_cv2(original_img)
            original_img = original_img.transpose(2, 0, 1)
